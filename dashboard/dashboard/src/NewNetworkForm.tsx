...
class DenseLayer implements Layer {
  ...
  constructor(
    units: number,
    activation: string | File | null,
    input_shape: string | File | null = null
  ) {
    this.layer = "Dense";
    this.units = units;
...
async function uploadFile(name: any, file_extentiom: any, file: any) {
  //TODO don't work fron front-end (works from swagger)
  const response = await fetch(
    `${UploadDataSourceEndPoint}/${name}/${file_extentiom}`,
    {
      method: "POST",
      headers: {
...
export class NewNetwork extends React.Component {
  ...
  addNetworkHandler = (event: any) => {
    ...
    postNetwork(localStorage.globalState).then((data) => {
  ...
  addLayerHandler = (name: string | File | null, layer: Layer) => {
    ...
    if (typeof localStorage.globalState !== "undefined") {
      ...
      network = { name: name, layers: [layer] };
    }
    ...
    return (
      <form onSubmit={this.addDenseLayerHandler}>
        ...
        <div className="row">
          ...
          <div className="col-75">{getActivation()}</div>
        </div>
        <div className="row">
          <div className="col-25">
          ...
          <div className="col-75">
            <input name="input_shape" type="text" />
          </div>
        </div>
    ...
    return (
      <form onSubmit={this.addConv2DLayerHandler}>
        ...
        <div className="row">
          ...
          <div className="col-75">
            <input name="kernel_size" type="text" />
          </div>
        </div>
        <div className="row">
          <div className="col-25">
    ...
    return (
      <form onSubmit={this.addMaxPooling2DLayerHandler}>
        ...
        <div className="row">
          ...
          <div className="col-75">
            <input name="pool_size" type="text" />
          </div>
        </div>
        <div className="row">
          <div className="col-25">
  ...
  chooseFrom() {
    if (
      typeof localStorage.layerName === "undefined" ||
  ...
  render() {
    return (
      <div className="container">
        {this.chooseFrom()}
        <br />
        <button id="add_network_button" onClick={this.addNetworkHandler}>
          Add new network
        </button>
        <button id="clear_network_button" onClick={this.clearNetworkHandler}>
          Clear network
        </button>
        <br />
        <br />
        <textarea
          id="new_layer_details"
          rows={4}
          cols={50}
          value={this.getTempNetworkDetails()}
          readOnly
        ></textarea>
        {this.uploadFile()}
      </div>

    ...
    async def message_handler(msg):
        try:
            ...
            if wait_for_response:
                logger.info(
                    f"Response from service'{data['service_name']}': {response}"
                )
...
def send_message(
    service_name: str, data: json, logger, config, wait_for_response: bool = True
):
    responses = Queue()

    async def call_service(
        nats_host: str, nats_port: str, service_topic: str, response_topic: str, loop
    ):
        nc = NATS()

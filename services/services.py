...
def new_network_service(values: json) -> str:
    ...
    response = {"Message": f"New Network {values['name']} added."}
    return json.dumps(response)


def compile_network(values: json) -> str:
    keras_wrapper.compile(
        model_name=values["name"],
        optimizer=values["optimizer"],
        loss=values["loss"],
        metrics=values["metrics"],
    )
    response = {"Message": f"Network {values['name']} compiled."}
    return json.dumps(response)
...
def train_network(values: json) -> str:
    ...
    response = {"Message": f"Network {values['name']} training complete."}
    return json.dumps(response)
...
def delete_network(values: json) -> str:
    ...
    response = {"Message": f"Network {values['name']} deleted."}
    return json.dumps(response)
...
def evaluate_network(values: json) -> str:
    ...
    test_loss, test_acc = keras_wrapper.evaluate(
        model_name=values["name"], test_data=test_data, test_labels=test_labels
    )
    response = {"Test_accuracy": test_acc, "Test_loss": test_loss}
    return json.dumps(response)
...
def model_to_json(network_name: str) -> dict:
    keras_wrapper.serialize_model(model_name=network_name, path=f"{network_name}.hdf5")
    return {
        "name": network_name,
...
def synchronize_data(*args):
    for network_name in keras_wrapper.get_models_names():
        if not mongo_wrapper.get_by_name(network_name):
            ...
            keras_wrapper.get_model(network_name).update_time_stamp

...
with open(
    path.join(current_dir, "../resources/compile_network_json.json")
) as json_file:
...
def add_new_network():
    ...
    new_network_response = requests.post(
        url=new_network_end_point,
        json=new_network_json,
...
def test_add_network(add_new_network):
    ...
    assert (
        add_new_network.json()["Message"]
        == f"New Network {new_network_json['name']} added."
    )


def test_get_networks():
    get_networks_end_point = f"{host}/networks"
    get_networks_response = requests.get(
        url=get_networks_end_point, headers={"content-type": "application/json"}
    )

    assert get_networks_response.status_code is 200
    assert {
        "name": new_network_json["name"],
        "compiled": False,
        "trained": False,
    } in get_networks_response.json()["Networks"]


def test_get_data_sources():
    get_data_sources_end_point = f"{host}/data-sources"
    get_networks_response = requests.get(
        url=get_data_sources_end_point, headers={"content-type": "application/json"}
    )
...
def test_compile_network():
    ...
    compile_network_response = requests.post(
        url=end_point,
        json=compile_network_json,
        headers={"content-type": "application/json"},
    )

    assert compile_network_response.status_code is 200
    assert (
        compile_network_response.json()["Message"]
        == f"Network {new_network_json['name']} compiled."
    )
...
def test_train_network():
    ...
    requests.post(
        url=end_point_compile,
        json=compile_network_json,
        headers={"content-type": "application/json"},
    )

    train_network_response = requests.post(
        url=end_point_train,
        json=train_network_json,
        headers={"content-type": "application/json"},
    )

    assert train_network_response.status_code is 200
    assert (
        train_network_response.json()["Message"]
        == f"Network {new_network_json['name']} training complete."
    )
...
def test_evaluate_network():
    ...
    evaluate_network_response = requests.post(
        url=end_point_evaluate,
        json={"name": train_network_json["name"]},
        headers={"content-type": "application/json"},
    )
...
def test_get_network_training_history():
    ...
    get_history_response = requests.get(
        url=end_point_get_history, headers={"content-type": "application/json"}
    )
...
def test_get_network_details():
    ...
    get_networks_response = requests.get(
        url=get_network_details_end_point, headers={"content-type": "application/json"}
    )
...
def test_health_check():
    ...
    get_networks_response = requests.get(
        url=health_check_end_point, headers={"content-type": "application/json"}
    )
...
def test_get_plots():
    ...
    loss_response = requests.get(
        url=end_point_loos_plt,
        json=train_network_json,
        headers={"content-type": "application/json"},
    )

    acc_response = requests.get(
        url=end_point_accuracy_plt,
        json=train_network_json,
        headers={"content-type": "application/json"},
    )

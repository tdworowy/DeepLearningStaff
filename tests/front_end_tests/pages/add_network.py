...
class AddNetworkPage:
    ...
    def __init__(
        self, url: str, logger: TestsLogger, web_driver_wrapper: WebDriverWrapper
    ):
        self.logger = logger
        self.url = url
        self.web_driver_wrapper = web_driver_wrapper
        self.add_layer_page_factory = AddLayerPageFactory(
            logger=self.logger, web_driver_wrapper=self.web_driver_wrapper
        )
    ...
    def click_add_layer_button(self) -> AddNetworkPage:
        ...
        self.web_driver_wrapper.find_element(*AddNetworkPage.add_layer_button).click()
        return self

    def click_add_network_button(self) -> AddNetworkPage:
        self.logger.log().info(f"Click add network button")
        self.web_driver_wrapper.find_element(*AddNetworkPage.add_network_button).click()
        return self

    def clear_layers(self):
        self.logger.log().info(f"Clear layers")
        self.web_driver_wrapper.find_element(
            *AddNetworkPage.clear_network_button
        ).click()
        return self
    ...
    def choose_layer_type(self, layer_type: str):
        ...
        Select(
            self.web_driver_wrapper.find_element(*AddNetworkPage.layer_select)
        ).select_by_value(layer_type)
        self.web_driver_wrapper.find_element(
            *AddNetworkPage.choose_layer_button
        ).click()
    ...
    def generate_output_json(self, name: str, layers: list) -> dict:
        ...
        return {
            "name": name,
            "layers": [
                self.add_layer_page_factory.get_add_layer_page(
                    layer.type
                ).generate_output_json(layer)
                for layer in layers
            ],
        }
    ...
    def assert_layer(self, expected_name: str, expected_layers: list):
        ...
        actual_value = self.web_driver_wrapper.find_element(
            *AddNetworkPage.layers_details
        ).text
        ...
        ), f"{actual_value} is not equal to {expected_value}"

    def check_networks_lists(self, names: list):
        self.logger.log().info(f"Check if networks f{names} exists")
        for name in names:
            try:
                self.web_driver_wrapper.wait_for_element(
                    By.XPATH, f'//*[@id="{name}"]/td/label'
                )
